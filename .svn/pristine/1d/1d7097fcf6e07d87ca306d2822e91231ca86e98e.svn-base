    ...
    <div class="main-serice" style="width: 98%;height: 400px;margin: 30px auto;clear: both;overflow: auto;border: 1px solid #ccc">
        ...
        <div class="first-tabcont shenke-b1">
            ...
            <div class="line1 clearfix">
             ...
             <div class="line1 clearfix">
                <div class="lineleft usufructArea">
                    <label>使用权面积：</label>
                    <span>fafsd</span>
                </div>
            ...
            <div class="line1 clearfix">               
                <div class="lineright amortizationState">
                    <label>摊销状态：</label>
                    <span>fafsd</span>
                </div>
        ...
        <div class="first-tabcont shenke-b1">
            ...
            <div class="line1 clearfix">
                <div class="lineleft landCertificate">
                    <label>土地证载明面积：</label>
                    <span>fafsd</span>
                </div>
                <div class="lineright singleArea">
                    <label>独用面积：</label>
                    <span>fafsd</span>
                </div>
            </div>
            <div class="line1 clearfix">
                <div class="lineleft apportionArea">
                    <label>分摊面积：</label>
                    <span>fafsd</span>
                </div>
                <div class="lineright usufructType">
                    <label>使用权类型：</label>
                    <span>fafsd</span>
                </div>
            </div>
            <div class="line1 clearfix">
                <div class="lineleft ownerLand">
                    <label>土地所有权人：</label>
                    <span>fafsd</span>
                </div>
            ...
            <div class="line1 clearfix">
                ...
                <div class="lineright landGradation">
                    <label>土地级次：</label>
                    <span>fafsd</span>
                </div>
            </div>
        ...
        <div class="first-tabcont shenke-b1">
			...
			<div class="line1 clearfix">
                ...
                <div class="lineright lendUnit">
                    <label>出借(出租)单位：</label>
                    <span>fafsd</span>
                </div>
           </div>
...
<script>
    ...
    var grid8=mini.get("form9");
			...
			//财务信息
				...
				//使用权面积
				var syarea = text.asset.usufructArea; 
				$('.first-tabcont .line1 .usufructArea span').html(syarea);
				...
				//摊销状态
				var txstau = text.asset.amortizationState; 
				$('.first-tabcont .line1 .amortizationState span').html(txstau);
			...
			//购置信息	
				...
				//地类（用途）
				var landyt = text.asset.landType; 
				$('.first-tabcont .line1 .landType span').html(landyt);
				...
				//地号
				var dh = text.asset.landNum; 
				$('.first-tabcont .line1 .landNum span').html(dh);
				//坐落位置
				var zlwz = text.asset.position; 
				$('.first-tabcont .line1 .position span').html(zlwz);

				//土地证载明面积
				var landzzm = text.asset.landCertificate; 
				$('.first-tabcont .line1 .landCertificate span').html(landzzm);
				
				//独用面积
				var dymj = text.asset.singleArea; 
				$('.first-tabcont .line1 .singleArea span').html(dymj);
				//分摊面积
				var ftmj = text.asset.apportionArea; 
				$('.first-tabcont .line1 .apportionArea span').html(ftmj);
				//使用权类型
				var syqlx = text.asset.usufructType; 
				$('.first-tabcont .line1 .usufructType span').html(syqlx);
				
				//土地所有权人
				var landown = text.asset.ownerLand; 
				$('.first-tabcont .line1 .ownerLand span').html(landown);
				...
				//土地级次
				var landjc = text.asset.landGradation; 
				$('.first-tabcont .line1 .landGradation span').html(landjc);
				...
				//权属年限
				var qsyear = text.asset.ownershipYears; 
				$('.first-tabcont .line1 .ownershipYears span').html(qsyear);
				
				//权属证书编号
				var qszsnum = text.asset.ownershipNumber; 
				$('.first-tabcont .line1 .ownershipNumber span').html(qszsnum);
				
				//权属性质
				var qsxz = text.asset.ownershipNature; 
				$('.first-tabcont .line1 .ownershipNature span').html(qsxz);
				//权属证书发证时间
				var qstime = ChangeDateFormat(text.asset.ownershipDate); 
				$('.first-tabcont .line1 .ownershipDate span').html(qstime);
			...
			//使用信息
				...
				//自用面积
				var selfmj = text.asset.selfUseArea; 
				$('.first-tabcont .line1 .selfUseArea span').html(selfmj);
				//自用价值
				var selfjz = text.asset.selfUseValue; 
				$('.first-tabcont .line1 .selfUseValue span').html(selfjz);
				
				//后勤：自用面积
				var hqmj = text.asset.logisticArea; 
				$('.first-tabcont .line1 .logisticArea span').html(hqmj);
				//土地闲置面积
				var landxzmj = text.asset.unusedArea; 
				$('.first-tabcont .line1 .unusedArea span').html(landxzmj);
				
				//出借面积
				var lendmj = text.asset.lendArea; 
				$('.first-tabcont .line1 .lendArea span').html(lendmj);
				//出借价值
				var lendjz = text.asset.lendValue; 
				$('.first-tabcont .line1 .lendValue span').html(lendjz);
				
				//出租面积
				var rentmj = text.asset.rentedArea; 
				$('.first-tabcont .line1 .rentedArea span').html(rentmj);
				//出租价值
				var rentjz = text.asset.rentalValue; 
				$('.first-tabcont .line1 .rentalValue span').html(rentjz);
				
				//对外投资面积
				var dwmj = text.asset.investmentArea; 
				$('.first-tabcont .line1 .investmentArea span').html(dwmj);
				//对外投资价值
				var dwjz = text.asset.investmentValue; 
				$('.first-tabcont .line1 .investmentValue span').html(dwjz);
				
				//其他面积
				var qtmj = text.asset.othUseArea; 
				$('.first-tabcont .line1 .othUseArea span').html(qtmj);
				//其他价值
				var qtjz = text.asset.otherValue; 
				$('.first-tabcont .line1 .otherValue span').html(qtjz);
				
				//出借（出租）单位
				var cjzunit = text.asset.lendUnit; 
				$('.first-tabcont .line1 .lendUnit span').html(cjzunit);
			...
			//其他信息
				...
				//资产单位管理名称
				var zcdwgl = text.asset.unitName; 
				$('.first-tabcont .line1 .unitName span').html(zcdwgl);
				//资产原分类名称
